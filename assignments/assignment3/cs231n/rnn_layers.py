...
def rnn_step_backward(dnext_h, cache):
    ...
    dupper = dnext_h * (1 - np.power(np.tanh(temp), 2))
    dx = dupper @ Wx.T
    dprev_h = dupper @ Wh.T
    dWx = x.T @ dupper
    dWh = prev_h.T @ dupper
...
def rnn_backward(dh, cache):
    ...
    for i in reversed(range(T)):
        dx_m, dnext_h, dWx_m, dWh_m, db_m = rnn_step_backward(dh[:, i, :] + dnext_h, grad_cache[i])
        dWx += dWx_m
        dWh += dWh_m
        db += db_m
        dx[:, i, :] = dx_m
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    dnext_c_plus = dnext_c + o * (1 - np.power(np.tanh(next_c), 2)) * dnext_h
    da = np.concatenate((dnext_c_plus * g * i * (1 - i),
                         dnext_c_plus * prev_c * f * (1 - f),
                         dnext_h * np.tanh(next_c) * o * (1 - o),
                         dnext_c_plus * i * (1 - np.power(g,2))), axis=1)
    
    dx = da @ Wx.T
    dprev_h = da @ Wh.T
    dprev_c = (dnext_c_plus) * f
    dWx = x.T @ da
    dWh = prev_h.T @ da
    db = np.sum(da, axis=0)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(T):
        prev_h, prev_c, grad_cache[i] = lstm_step_forward(x[:, i, :], prev_h, prev_c, Wx, Wh, b)
        h[:, i, :] = prev_h
...
def lstm_backward(dh, cache):
    ...
    for i in reversed(range(T)):
        dx_m, dprev_h, dprev_c, dWx_m, dWh_m, db_m = lstm_step_backward(dprev_h + dh[:, i, :], dprev_c, grad_cache[i])
        dWx += dWx_m
        dWh += dWh_m
        db += db_m
        dx[:, i, :] = dx_m
